...
//        String dateFormat = PropertiesUtil.getProperty("drquery.service/runtime.properties", "ocnosql.query.date.format");
    ...
    public BaseDTO processF31(CommonDRProcessRequest request, MetaModel viewMeta, final Map extendParams) throws ProcessException, BusinessException {
        ...
        String startTime = request.getParam("startTime");
        Date start = DateUtil.add(startTime, "yyyyMMddHH", 10, amplifier);
        String startTime_ = DateUtil.dateToString(start, "yyyyMMddHHmm");


        String endTime = request.getParam("endTime");
        Date end = DateUtil.add(endTime, "yyyyMMddHH", 10, amplifier);
        ...
        String sql = "";
        for (String month : months) {
            sql = "select START_TIME, APP_ID, SERVICE_HOST, ACCESS_MODE_ID, IMEI_SEG, UP_FLOW, DOWN_FLOW, FLOW "
                    + " from GPRS_" + month
                    + " where id >='" + startKey + "' and id <'" + stopKey + "'";
        ...
        sql = sql.substring(0, sql.lastIndexOf("union"));
        sql = "select * from (" + sql + ") t order by START_TIME";
        List<Map<String, String>> list = loadData(DefaultQueryParams.newBuilder().buildSQL(sql));
        long totalFlow = 0l;
        for(int i = 0; i < list.size(); i++) {
            totalFlow += Long.parseLong(list.get(i).get("FLOW"));
        }
        Map<Integer, Double> percentMap = new HashMap<Integer, Double>();
        for(int i = 0; i < list.size(); i++) {
            double percent = Long.parseLong(list.get(i).get("FLOW")) / totalFlow;
            percentMap.put(i, percent);
        }

        long diff = totalFlow - chargeFlow;
        long reserve = shareFlow(percentMap, diff, threhold, list);
        while(reserve > 0) {
            reserve = shareFlow(percentMap, reserve, threhold, list);
        }
        PageDTO dto = new PageDTO(list, list.size());
        return dto;
    }

    public long shareFlow(Map<Integer, Double> percentMap, long flow, int threhold, List<Map<String, String>> list) {
        long reserve = 0;
        for(int i = 0; i < list.size(); i++) {
            Map<String, String> record = list.get(i);
            long shareFlow = new Double(flow * percentMap.get(i)).longValue();
            if(shareFlow > threhold) {
                reserve = shareFlow - threhold;
                shareFlow = threhold;
            }
            record.put("FLOW", (Long.parseLong(list.get(i).get("FLOW")) + shareFlow) + "");
        }
        return reserve;
    }
